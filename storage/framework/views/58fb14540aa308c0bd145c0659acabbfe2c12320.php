...
<div class="row" id="prof" >
	<div class="col-lg-12">
		<div class="dpr-well" style="padding-left:40px; <?php if(Auth::guest()): ?>
<?php else: ?>
<?php if(Auth::user()->type==1): ?>
	<?php else: ?>
<?php if(Auth::user()->approved==1): ?>
<?php elseif(Auth::user()->approved==0): ?>

	background-image:url('<?php echo e(asset('img/rejected.jpg')); ?>');
...
<?php endif; ?>
<?php endif; ?>
<?php endif; ?> background-size:100%;">
			<div class="img-responsive">
				<img class="pull-left" alt="user_profile" src="<?php echo e(asset('upload')); ?>/<?php echo e($user->image); ?>" style="height:150px; width:150px;">
			</div>
			<div class="data">
				...
				<p class="lead" id="dpr-pf-name">
					...
					<br><b>Application Status:</b> <?php if($user->approved=="0"): ?><span class="label label-warning">Pending</span>
					...
					<strong style="margin-left:150px;" class="text-danger"><b>Reference Number:</b> <?php echo e($user->ref_num); ?></strong>
					<br>
    ...
    <a  onclick="sendmail()" id="noprint" style="color:black; cursor:pointer; text-decoration:none;"><i class="fa fa-share"></i> Send as Email</a>
					...
					<B>Score:</b> <b style="color:green"><?php echo e(round(Auth::user()->exam_init/5,2)*100); ?>%</b></div>
					<?php else: ?>	
					<?php if(Auth::user()->type=="1"): ?>
                    <?php else: ?>
					<?php if(Auth::user()->complete=="1"	): ?>					
   ...
   data-keyboard="false"><i class="fa fa-file-text"></i><b class="tab blink">Take Test</b></a> 
				...
				<?php else: ?>
					<?php  $margin=60;  ?>
				<?php endif; ?>
				<div id="demo" style="margin-left:145px; margin-top:<?php echo e($margin); ?>px;"></div>
				<br><span class='text-danger'><b><?php if(Auth::user()->type==1): ?> <?php else: ?> <?php if(Auth::user()->approved==1): ?> Successful candidates should come along with their registration slip to the examination centre. <?php elseif(Auth::user()->approved==0): ?> This is a Temporary Copy , Do Not come with this copy to the examination hall,Please Check back later for the Permanent Copy <?php else: ?>  Sorry Your Application Has Not Been Approved, PLease do not come to the Examination center with this slip <?php endif; ?> <?php endif; ?></b></span>
					
	
			</div>
			
		</div>
	<?php if(Auth::user()->complete==1 && Auth::user()->type==0 ): ?></div><?php endif; ?>
...
<div class="">
	...
	<?php if(Auth::user()->complete=="0"): ?>
		<?php if(Auth::user()->type=="1"): ?>
			<?php else: ?>
    <a  onclick="submit2('<?php echo e(Auth::user()->id); ?>')" class="btn btn-template-main"><i class="fa fa-paper-plane-o"></i> Submit & Print</a>
	<a style="cursor:pointer" onclick="reset()"  class="btn btn-danger"><i class="fa fa-repeat"></i> Restart Application</a>
